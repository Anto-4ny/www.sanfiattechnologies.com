...
// Ensure user is logged in and fetch their payment status
...
const showPaymentPopup = () => {
    ...
    // Show payment section when the user clicks "Activate Account"
    ...
    document.getElementById('pay-btn').addEventListener('click', async () => {
        ...
        if (phoneNumber.match(/^07\d{8}$/) && email) {
            initiatePayment(phoneNumber, email);
        } else {
...
// Initiate payment
const initiatePayment = (phoneNumber, email) => {
    fetch('https://www-sanfiattechnologies-com.vercel.app/api/pay', {
        method: 'POST',
