...
const fetchUserProfile = async (userId) =>
    { 
    try { const userDocRef = db.collection('users').doc(userId); 
const userSnapshot = await userDocRef.get(); 
    ...
    document.getElementById('user-package-status').textContent = userData.packageStatus || 'No active package'; 
        ...
        else { console.log('No user data found in Firestore.'); 
             } 
        } 
    catch (error) 
    {
...
const saveUserProfile = async (userId) => { try 
{
    const userDocRef = db.collection('users').doc(userId); 
    const updatedData = { fullName: document.getElementById('user-full-name').textContent, 
...
document.getElementById('upload-user-profile-pic').addEventListener('change', async (event) => 
    {
        ...
        {
            const user = auth.currentUser; const storageRef = firebase.storage().ref();
            // Upload profile picture 
            const profilePicRef = storageRef.child(`profile_pictures/${user.uid}.jpg`); await profilePicRef.put(file); 
